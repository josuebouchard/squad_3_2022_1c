...
from .exceptions import DeadlineBeforeCreationDateException
...
class TicketService:
    ...
    def addEmployees(self, employeeIDs, ticketID):
        return self._employee_service.addEmployees(employeeIDs, ticketID)
    ...
    def _assert_deadline_is_valid(
        self,
        deadline: DateTime,
        creationDate: DateTime = datetime.now(timezone.utc),
    ) -> None:
